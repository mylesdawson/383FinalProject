...
func gather_trending(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("Access-Control-Allow-Origin", "*")

	keys, ok := req.URL.Query()["id"]
	if !ok || len(keys[0]) < 1 {
		http.Error(w, "Enter query parameter at the end of the url: ?id=x", http.StatusBadRequest)
		return
	}
	place := keys[0]

	cmd := exec.Command("/usr/bin/python3", "./trending.py", place)
	res, err := cmd.CombinedOutput()

	if err != nil {
		http.Error(w, "Error when running python script to get trending tweet data", http.StatusInternalServerError)
		return
	}
...
func main() {
	...
	r.HandleFunc("/trending", gather_trending)
